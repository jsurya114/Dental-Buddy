...
import { fetchUsers, toggleUser, deleteUser } from "../redux/userSlice";
...
const UserManagement = () => {
    ...
    const getRoleDisplayName = (roleCode) => {
        ...
        return role?.displayName || roleCode;
    };

    const handleToggle = async (id) => {
        try {
            setActionLoading(id);
            await dispatch(toggleUser(id)).unwrap();
        } catch (err) {
            alert(err || "Failed to toggle user");
        } finally {
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-gray-500">Create and manage user accounts for your clinic</p>
                    </div>
                    <button
                        onClick={() => navigate("/clinic-admin/users/create")}
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {users.map((user) => (
                                    <tr key={user._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <code className="text-sm bg-gray-100 px-2 py-1 rounded">{user.loginId}</code>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-end gap-2">
                                                <button
                                                    onClick={() => navigate(`/clinic-admin/users/edit/${user._id}`)}
